...
class PascalVocData(object):
    ...
    def __init__(self, conf):
        ...
        self.conf = conf
        # 数据形式
        self.image_height = self.conf.input_size
        self.image_width = self.conf.input_size
        ...
        if not self.conf.is_training:
            # batch方法需要明确
            resized_image, resized_annotation = DataAugmentation.scale_randomly_image_with_annotation_with_fixed_size_output(
            ...
            # 数据增强
            image_op, annotation_op = DataAugmentation.flip_randomly_left_right_image_with_annotation(image_op, annotation_op)
            image_op = DataAugmentation.distort_randomly_image_color(image_op)
            resized_image, resized_annotation = DataAugmentation.scale_randomly_image_with_annotation_with_fixed_size_output(
                image_op, annotation_op, [self.image_height, self.image_width])
            resized_annotation = tf.cast(resized_annotation, tf.uint8)
